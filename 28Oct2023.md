...
Put this line of code before the ```for``` loop then edit the ```for``` loop to use the variable instead of the number of sides. Have a go at doing this yourself, before looking at the solution.
